...
def print_help():
    ...
    print (
        "\tputdoc [document input path] [collection address] [title] [description]")
    print ("\tlistcollections")
    print (
        "\tlistversions [collection address] [(optional) 'documents' to print document_ids]")
    print ("\tlisten")
...
def get_doc_file(document_hash, document_output_filename):
    ...
    print ("File with URL " + document_hash +
           " written to " + document_output_filename)
...
def list_collection_version(collection_address, document_ids):
    ...
    for version in versions:
        print("\t" + version.root_hash +
              "\t" + str(version.collection_version))
...
def put_document(file_path, collection_address, title, description):
    ...
    print (
        "Allow up to 10 minutes for file to propogate on the freenet network")
...
def publish_collection(address_password, collection_address):
    ...
    if collection is not None:
        ...
        collection_handler.publish_collection(
            collection, config.MAIN_CHANNEL_ADDRESS, address)
